...
function triggerDayChoose(){if (options.minYear != null && options.minMonth != null && options.minDay != null){var choosenYear = $(this).data('year'); var choosenMonth = $(this).data('month'); var choosenDay = $(this).data('day-of-month'); var date = new Date(choosenYear, choosenMonth, choosenDay); if (parseInt(choosenYear) <= parseInt(options.minYear)){if (parseInt(choosenMonth) < parseInt(options.minMonth) || (parseInt(choosenMonth) == parseInt(options.minMonth) && parseInt(choosenDay) < parseInt(options.minDay))){alert(options.minDayMessage); return!1}}}
if (options.maxYear != null && options.maxMonth != null && options.maxDay != null){var choosenYear = $(this).data('year'); var choosenMonth = $(this).data('month'); var choosenDay = $(this).data('day-of-month'); var date = new Date(choosenYear, choosenMonth, choosenDay); if (parseInt(choosenYear) <= parseInt(options.maxYear)){if (parseInt(choosenMonth) < parseInt(options.maxMonth) || (parseInt(choosenMonth) == parseInt(options.maxMonth) && parseInt(choosenDay) < parseInt(options.maxDay))){alert(options.maxDayMessage); return!1}}}
if (options.addUniqueClassOnClick){var uniqueClass = options.addUniqueClassOnClick; $('.' + uniqueClass).removeClass(uniqueClass); $(this).addClass(uniqueClass)}
$calendar.data('day-of-month', $(this).data('day-of-month')); $calendar.data('month', $(this).data('month')); $calendar.data('year', $(this).data('year')); $calendar.trigger('jqyc.dayChoose')}
return methods.init.apply(this, arguments)} else{$.error('Method ' + options + ' does not exist on jQuery.tooltip')}}; var currentDate = new Date(); $.fn.calendar.defaults = {showHeaders:!0, startYear:currentDate.getFullYear(), maxYear:null, maxDay:null, maxMonth:null, maxDayMessage:'You can not choose day from future', minYear:null, minDay:null, minMonth:null, minDayMessage:'You can not choose day from past', boostrapVersion:4, cols:12, colsSm:6, colsMd:4, colsLg:3, colsXl:3, addUniqueClassOnClick:!1, l10n:{jan:"January", feb:"February", mar:"March", apr:"April", may:"May", jun:"June", jul:"July", aug:"Augst", sep:"September", oct:"October", nov:"November", dec:"December", mn:"Mn", tu:"Tu", we:'We', th:'Th', fr:'Fr', sa:'Sa', su:'Su'}}; function createWholeCalendar(settings, $this){$this.empty(); var bV = parseInt(settings.boostrapVersion); if (bV == 3){colColXs2 = 'col-xs-2'; colColXs3 = 'col-xs-3'; colColXs4 = 'col-xs-4'} else{colColXs2 = 'col'; colColXs3 = 'col'; colColXs4 = 'col'}
...
$calendar.trigger('jqyc.changeYear'); biggestHeight = 0; $('.jqyc-month').each(function(){var height = $(this).height(); if (height > biggestHeight){biggestHeight = height}}); $('.jqyc-month').css('height', biggestHeight); var choosenYear = $calendar.data('year'); var choosenMonth = $calendar.data('month'); var choosenDayOfMonth = $calendar.data('day-of-month'); $calendar.find('.jqyc-not-empty-td[data-day-of-month="' + choosenDayOfMonth + '"][data-month="' + choosenMonth + '"][data-year="' + choosenYear + '"]').addClass(settings.addUniqueClassOnClick)}
...
i++; d++}
return{monthHTMLString:monthHTMLString, firstDayOfPreviousMonth:(i % 7)}}}(jQuery))
