...
(function($){var methods={init:function(options){},destroy:function(){this.empty()},hide:function(){this.hide()},recalcHeight:function(){recalcHeight()},clearText:function(year,month,day){this.find('.jqyc-not-empty-td[data-day-of-month="'+day+'"].jqyc-not-empty-td[data-month="'+month+'"].jqyc-not-empty-td[data-year="'+year+'"]').removeClass('jqyc-appended').text(day);recalcHeight()},clearTextFromAll:function(){this.find('.jqyc-appended').each(function(){$(this).removeClass('jqyc-appended').text($(this).data('day-of-month'))});recalcHeight()},appendText:function(text,year,month,day,classes="small text-danger"){this.find('.jqyc-not-empty-td[data-day-of-month="'+day+'"].jqyc-not-empty-td[data-month="'+month+'"].jqyc-not-empty-td[data-year="'+year+'"]').addClass('jqyc-appended').append(' <span class="'+classes+'">'+text+'</span>');recalcHeight()}};$.fn.calendar=function(options){$calendar=this;if(methods[options]){return methods[options].apply(this,Array.prototype.slice.call(arguments,1))}else if(typeof options==='object'||!options){settings=$.extend({},$.fn.calendar.defaults,options);createWholeCalendar(settings,this);$calendar.find('.jqyc-prev-year').on("click",showPreviousYear,);$calendar.find('.jqyc-next-year').on("click",showNextYear);$calendar.find('.jqyc-not-empty-td').on("click",triggerDayChoose);function showPreviousYear(){var currentYear=parseInt($(this).parent().parent().data('currentYear'));settings.startYear=settings.startYear-1;createWholeCalendar(settings,$calendar);$calendar.find('.jqyc-prev-year').on("click",showPreviousYear);$calendar.find('.jqyc-next-year').on("click",showNextYear);$calendar.find('.jqyc-not-empty-td').on("click",triggerDayChoose);$calendar.trigger('jqyc.changeYearToPrevious');$(this).parent().parent().data('currentYear',currentYear)}
    ...
    function triggerDayChoose(){var choosenYear=$(this).data('year');var choosenMonth=$(this).data('month');var choosenDay=$(this).data('day-of-month');if(settings.minYear!=null&&settings.minMonth!=null&&settings.minDay!=null){var date=new Date(choosenYear,choosenMonth,choosenDay);if(parseInt(choosenYear)<=parseInt(settings.minYear)){if(parseInt(choosenMonth)<parseInt(settings.minMonth)||(parseInt(choosenMonth)==parseInt(settings.minMonth)&&parseInt(choosenDay)<parseInt(settings.minDay))){if(settings.minDayMessage!=null){alert(settings.minDayMessage)}
        $calendar.trigger('jqyc.notMinDayChoose');$calendar.trigger('jqyc.outOfRangeDayChoose');return!1}}}
        if(settings.maxYear!=null&&settings.maxMonth!=null&&settings.maxDay!=null){var date=new Date(choosenYear,choosenMonth,choosenDay);if(parseInt(choosenYear)<=parseInt(settings.maxYear)){if(parseInt(choosenMonth)<parseInt(settings.maxMonth)||(parseInt(choosenMonth)==parseInt(settings.maxMonth)&&parseInt(choosenDay)<parseInt(settings.maxDay))){if(settings.maxDayMessage!=null){alert(settings.maxDayMessage)}
            $calendar.trigger('jqyc.notMaxDayChoose');$calendar.trigger('jqyc.outOfRangeDayChoose');return!1}}}
        if(settings.mode=='rangepicker'){if($calendar.data('rangepicker-start-choosen')!=!0){$calendar.data('rangepicker-start-day-of-month',choosenDay);$calendar.data('rangepicker-start-month',choosenMonth);$calendar.data('rangepicker-start-year',choosenYear);$calendar.data('rangepicker-start-choosen',!0);$(this).addClass('jqyc-start-day-of-month')}else if($calendar.data('rangepicker-end-choosen')!=!0){if(settings.maxDaysToChoose){var startDay=[$calendar.data('rangepicker-start-day-of-month'),$calendar.data('rangepicker-start-month'),$calendar.data('rangepicker-start-year')];var endDay=[choosenDay,choosenMonth,choosenYear];var startDate=new Date(startDay[2],startDay[1],startDay[0]);var endDate=new Date(endDay[2],endDay[1],endDay[0]);var daysBettweenDates=Math.round((startDate-endDate)/(1000*60*60*24));if((settings.maxDaysToChoose-1)<Math.abs(daysBettweenDates)){if(settings.maxDaysToChooseMessage){alert(settings.maxDaysToChooseMessage+settings.maxDaysToChoose)}
    ...
    if(settings.colsSm!=12){$this.find('.jqyc-months').addClass('row');$this.find('.jqyc-month').addClass('col-sm-'+settings.colsSm)}
    if(settings.colsMd!=12){$this.find('.jqyc-months').addClass('row');$this.find('.jqyc-month').addClass('col-md-'+settings.colsMd)}
    if(settings.colsLg!=12){$this.find('.jqyc-months').addClass('row');$this.find('.jqyc-month').addClass('col-lg-'+settings.colsLg)}
    ...
    function jqycGetMonthHTMLStringWithData(firstDay,month,year,days=31){if(firstDay==0){firstDay=7}
        var monthHTMLString='';var d=1;var i=1;while(d<=days){if(i%7==1){monthHTMLString=monthHTMLString+'<tr class="jqyc-tr jqyc-tbody-tr">'}
            if(i<firstDay){d--;monthHTMLString=monthHTMLString+'<td class="jqyc-empty-td jqyc-td"></td>'}else{monthHTMLString=monthHTMLString+'<td class="jqyc-not-empty-td jqyc-td jqyc-day-'+d+' jqyc-day-of-'+month+'-month" data-month="'+month+'" data-day-of-month="'+d+'" data-year="'+year+'">'+d+' </td>'}
            if(i%7==0){monthHTMLString=monthHTMLString+'</tr>'}
            i++;d++}
        return{monthHTMLString:monthHTMLString,firstDayOfPreviousMonth:(i%7)}}
    function recalcHeight(){biggestHeight=0;$('.jqyc-month').css('height','inherit');$('.jqyc-month').each(function(){var height=$(this).height();if(height>biggestHeight){biggestHeight=height}});$('.jqyc-month').css('height',biggestHeight)}
